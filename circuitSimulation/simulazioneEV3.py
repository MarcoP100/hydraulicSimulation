...
import shutil
import numpy as np
from fmpy import simulate_fmu, extract, read_model_description, instantiate_fmu, dump
...
# Instanzia l'FMU
#fmi_logging = True  # Abilita o disabilita il logging FMI
fmu_instance = instantiate_fmu(unzipdir, 
                               model_description, 
                               'ModelExchange' 
...
#fmu_instance.setDebugLogging(True,['logAll'])
#fmu_instance.setupExperiment(startTime=0.0)

#continuous_states = fmu_instance.getContinuousStates()
...
current_time = 0.0
time_end = 0.1  # 10 secondi di simulazione
...
try:
    ...
    for block in range(num_blocks):
        ...
        try:
            #continuous_states = fmu_instance.setContinuousStates()
            ...
            model_state = simulate_fmu(fmu_path, 
                                    start_time=start_time, 
                                    stop_time=end_time, 
                                    step_size=dt_model, 
                                    input=inputs_structured,
                                    #initialize=False, 
                                    output=['currentFeedback'], 
            ...
            feedback = block_feedback[-1]
            #print(f"Feedback: {feedback}")  # Debug feedback
        ...
        #    block_feedback = model_state['currentFeedback']
            ...
            log_file.write(f"Tempo: {block_time[-1]}, Feedback: {feedback}, Duty Cycle: {duty_cycle}\n")
